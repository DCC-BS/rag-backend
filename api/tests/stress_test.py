...
async def ranking():
    ...
    test_docs = ["The fish is 10 dollars", "I dont know", "Der Fisch ist Gratis", "Ich esse Gemüse"]
    ...
    async with bentoml.AsyncHTTPClient("http://localhost:50001") as client:
        if await client.is_ready():
            ...
            start_time = time.time()
            execution_times = await asyncio.gather(*tasks)  # Run tasks and get individual times
            end_time = time.time()

            total_execution_time = sum(execution_times)
            print(f"Total execution time for all ranking calls: {total_execution_time:.4f} seconds")
            print(f"Time from start to finish of all ranking calls: {end_time-start_time:.4f} seconds")
        else:
