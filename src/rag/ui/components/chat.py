...
from rag.utils.stream_response import StreamResponseType
...
def process_stream(
    ...
    if is_resume:
        ...
        stream = st.session_state[CONVERSATIONAL_PIPELINE].stream_query(prompt, "Sozialhilfe", thread_id)

    for chunk in stream:
        if chunk.type == StreamResponseType.STATUS:
            flow_updates.append((chunk.message, chunk.decision if hasattr(chunk, "decision") else ""))
    ...
    status.update(
        label="Fertig!" if "interrupt_state" not in st.session_state else "Warte auf Feedback",
        state="complete",
    )
...
def manage_chat():
    ...
    if "interrupt_state" in st.session_state:
        ...
        if interrupt_type == "needs_information":
            st.info("Die Frage ist nicht eindeutig. Bitte geben Sie weitere Informationen ein:")
            additional_info = st.text_area("Zusätzliche Informationen")
            if st.button("Informationen senden"):
                with (
                    st.chat_message("assistant"),
                    st.status("Verarbeite zusätzliche Informationen...", expanded=True) as status,
                ):
                    process_stream(
                        prompt=None,
                        thread_id=thread_id,
            ...
            if accept_clicked:
                with st.chat_message("assistant"), st.status("Verarbeite Anfrage...", expanded=True) as status:
                    process_stream(
                        prompt=None,
                        thread_id=thread_id,
                        status=status,
        ...
        if st.session_state.get("modifying_query"):
            modified_query = st.text_area("Bearbeiten Sie die Frage:", value=st.session_state["original_query"])
            if st.button("Geänderte Frage senden"):
                with st.chat_message("assistant"), st.status("Verarbeite geänderte Frage...", expanded=True) as status:
                    process_stream(
    ...
    if prompt is not None:
        ...
        with st.chat_message("assistant"), st.status("Suche relevante Dokumente...", expanded=True) as status:
...
def _render_flow_updates(flow_updates):
    """Render the flow updates in an expander."""
    with st.expander("Arbeitsablauf", expanded=False):
        st.write("Details der einzelnen Arbeitsschritte im Arbeitsablauf:")
        for node_label, node_output in flow_updates:
            ...
            if node_label == "human_in_the_loop":
                ...
                continue
            _render_node_output(node_label, node_output)


def _render_node_output(node_label: str, node_output: Any) -> None:
    """Render a single node's output."""
...
def _render_assistant_message(message: dict[str, Any], relevant_docs: list[Any]) -> None:
    """Render an assistant's message with citations and potential feedback form."""
    ...
    for message in st.session_state[UI_RENDERED_MESSAGES]:
        with st.chat_message(message["role"]):
            if message["role"] == "assistant":
                ...
                _render_assistant_message(message, relevant_docs)
            else:
                st.markdown(message["content"])

            if message.get("flow_updates"):
                _render_flow_updates(message["flow_updates"])
